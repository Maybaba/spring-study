...
//
//    // 카카오 로그인 처리 서비스 로직
//    public void kakaoLogin(HashMap<String, Object> requestParams) {
//
//        // 토큰 발급 요청
//        String accessToken = getKakaoAccessToken(requestParams);
//    }
...
//         *             param1: 요청 URL
//         *             param2: 요청 방식 (get, post, put, patch, delete…)
//         *             param3: 요청 헤더와 요청 바디 정보 - HttpEntity로 포장해서 줘야 함 혹은 json을 넣을 수도 있음
//         *             param4: 응답결과(JSON)를 어떤 타입으로 받아낼 것인지 (ex: DTO로 받을건지 Map으로 받을건지)
//         */
//
//        //responseEntity 에 header랑 body 받을 수 있다.
//        ResponseEntity<AccessTokenDto> response = template.exchange(requestUri, HttpMethod.POST, entity, AccessTokenDto.class);// fetch는 클라이언트와 서버와 통신
//
////        log.debug("response: {}", response);
//
//        //json은 body에 위치하기 때문에 body 파싱
//        AccessTokenDto json = response.getBody();
//
//        log.debug( "json : {}", json);
//
...
//
//        return accessToken;
//    } //맵이 아닌 DTO로 받을 수도 있음 -> access token dto로 받을 수 있음
//
//
//    //발급받은 토큰으로 사용자 정보 가져오기
